...
function parentLayout.init()
	-- Preloading some configs
	allowMove, allowMoveConfig = config.getboolean("allowMoveCursorWhenNavigating", true),
...
if allowMoveConfig == false then
	stretchMarkerActions:registerProperty {
		...
		set_perform = function(self, parent)
			local message = initOutputMessage()
			reaper.SetEditCurPos(item_properties_macros.pos_relativeToGlobal(parent.marker.item, parent.marker.pos), true,
				true)
...
stretchMarkerActions:registerProperty {
	...
	set_perform = function(self, parent)
		local curpos = reaper.GetCursorPosition()
		reaper.SetEditCurPos(item_properties_macros.pos_relativeToGlobal(parent.marker.item, parent.marker.pos), false,
			false)
				...
				get = function(self)
					...
					do
						...
						local itemPosition, takePlayrate, itemLength =
							reaper.GetMediaItemInfo_Value(self.marker.item, "D_POSITION"),
					...
					message { label = string.format("%stretch marker %u of %s %s",
						({ [false] = "S", [true] = "Pulled s" })[
