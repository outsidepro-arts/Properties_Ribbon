...
return extstate._layout[string.format("searchinAction.%s.%s", utils.removeSpaces(searchinAction.objStrings[context]), key)]
end,
...
getId = function(obj) return string.format("%s with take %s", item_properties_macros.getItemID(obj), item_properties_macros.getTakeID(obj)) end
...
message:initType("Toggle this property to enable or disable the Lua patterns in search queries. The Lua patterns like RegExp patterns, so your search query can be more powerfull.", "Toggleable")
return message
end,
set_perform = function (self,parent)
local message = initOutputMessage()
...
__index = function (self, key)
return extstate._layout[string.format("searchbyPluginsAction.%s.%s", utils.removeSpaces(searchinAction.objStrings[context]), key)]
end,
__newindex = function (self, key, value)
extstate._layout[string.format("searchbyPluginsAction.%s.%s", utils.removeSpaces(searchinAction.objStrings[context]), key)] = value
end
})
