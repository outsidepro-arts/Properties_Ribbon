...
fxLayout.section = utils.removeSpaces(fxLayout.name)
...
fxLayout.ofCount = fxCount + fxRecCount
...
local fxGet = function(self)
	...
	if parmIdentification > 0 then
		...
		if self.parmIndex < capi.GetNumParams(self.fxIndex) - 3 then
...
local fxSet = function(self, direction)
	...
	if mode == 0 then
		...
		if self.parmIndex == (capi.GetNumParams(self.fxIndex) - 2 - deltaExists) or (deltaExists == 1 and self.parmIndex == (capi.GetNumParams(self.fxIndex) - 1
			)) then
...
setmetatable(fxLayout,{
	__index = function (self, rawFXId)
		local fxId = rawFXId
		if isnumber(fxId) and fxId > 0 then
			fxId = fxId - 1
		else
			return
		end
		if context == 0 and fxId >= fxCount and fxRecCount > 0 then
			fxId = 0x1000000 + (fxId - fxCount)
		end
		if getFormattedFXName(fxId) then
			...
			if fxName then
				...
				fxName = fxPrefix .. fxName
			end
			fxName = fxName .. ({ [true] = "", [false] = " (bypassed)" })[capi.GetEnabled(fxId)]
			fxName = fxName .. ({ [false] = "", [true] = " (offline)" })[capi.GetOffline(fxId)]
			local mayNextSub, mayPrevSub
			if (fxId + 1) == fxCount and fxRecCount > 0 then
				if capi.GetFXName(0x1000000) then
					mayNextSub = rawFXId + 1
				end
			else
				if capi.GetFXName(fxId + 1) then
					mayNextSub = rawFXId + 1
				end
			end
			if (fxId - 1) == (0x1000000 - 1) and fxCount > 0 then
				if capi.GetFXName(fxCount - 1) then
			...
			return setmetatable({
				...
				slIndex = rawFXId,
				nextSubLayout = mayNextSub,
				previousSubLayout = mayPrevSub,
				properties = setmetatable({}, {
					__index = function (self, paramId)
						if isnumber(paramId) and paramId > 0 then
							paramId = paramId - 1
						else
							return
						end
