...
local function composeMultipleTakeMessage(func, states, inaccuracy)
	...
	for k = 1, #items do
		...
		if (state ~= prevState and state == nextState) and (takeIDX ~= prevTakeIDX and takeIDX == nextTakeIDX) then
			message { value = string.format("takes from %s of %s ", getTakeID(items[k]):gsub("take ", ""), getItemID(items[k])) }
...
function lockProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the lock state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
...
function itemVolumeProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the relative of previous value will be applied for each item of."
			, 1)
	end
...
function muteItemProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the mute state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
	if istable(items) then
		message { label = "Mute" }
...
function loopSourceProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the loop source state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
	if istable(items) then
...
function itemAllTakesPlayProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the playing all takes state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
	if istable(items) then
		message { label = "Playing all takes" }
		message(composeMultipleItemMessage(function(item) return reaper.GetMediaItemInfo_Value(item, "B_ALLTAKESPLAY") end,
			self.states))
	else
...
function timebaseProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(string.format(' If the group of items has been selected, the value will enumerate only if these items have the same value. Otherwise, the timebase state will be set to "%s", then will enumerate this.'
			, self.states[0]), 1)
	end
	message { label = "Timebase" }
	if istable(items) then
		message(composeMultipleItemMessage(function(item) return reaper.GetMediaItemInfo_Value(item, "C_BEATATTACHMODE") end,
			self.states, 1))
	else
		local state = self.getValue(items)
...
function autoStretchProperty:get()
	local message = initOutputMessage()
	message:initType(string.format('Toggle this property to enable or disable auto-stretch selected item at project tempo when the item timebase is set to "%s".'
		, timebaseProperty.states[2]), "Toggleable")
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the auto-stretch state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
	message { label = "Auto-stretch at project tempo" }
	if istable(items) then
		message(composeMultipleItemMessage(function(item) return reaper.GetMediaItemInfo_Value(item, "C_AUTOSTRETCH") end,
			self.states))
	else
...
function itemSnapOffsetProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has selected, the relative depending on previous value will be set for each selected item."
			, 1)
	end
...
function groupingProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the group will be set to 1 first, then will begins enumerate up of."
			, 1)
	end
	if istable(items) then
...
fadeinShapeProperty.states = setmetatable({
	...
	__index = function(self, key)
		return string.format("Unknown fade type %s. Please create an issue with this fade type on the properties Ribbon github repository."
			, tostring(key))
	end
})
...
function fadeinShapeProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(string.format(' If the group of items has been selected, the value will enumerate only if these items have the same value. Otherwise, the shape state will be set to "%s", then will enumerate this.'
			, self.states[0]), 1)
	end
...
activeTakeProperty.extendedProperties:registerProperty {
	...
	get = function(self, parent)
		...
		if multiSelectionSupport then
			message:addType(" If the group of item has been selected, the every take of will get new specified name with ordered number."
				, 1)
		end
		if istable(items) then
...
function takeVolumeProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the relative of previous value will be applied for each item take of."
			, 1)
	end
	message { label = "Volume" }
	if istable(items) then
		...
		local state = self.getValue(items)
		message { objectId = string.format("%s %s", getItemID(items), getTakeID(items)), value = representation.db[state] }
	end
	return message
end
...
function takePanProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the relative of previous value will be applied for each item active take of."
			, 1)
	end
	message { label = "Pan" }
	if istable(items) then
...
function takePhaseProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the phase polarity state will be set to oposite value depending of moreness takes of items with the same value."
			, 1)
	end
	message { label = "Phase" }
	if istable(items) then
...
function takeChannelModeProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(string.format(' If the group of items has been selected, the value will enumerate only if selected items have the same value. Otherwise, the channel mode state will be set to "%s", then will enumerate this.'
			, self.states[0]), 1)
	end
	message:addType(" Toggle this property to switch between channel mode categories.", 1)
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the category will define by first selected item take and next category will be switch for selected selected items."
			, 1)
	end
...
function takePlayrateProperty:get()
	...
	if istable(items) then
		...
		local state = self.getValue(items)
		message { objectId = string.format("%s %s", getItemID(items), getTakeID(items)), value = representation.playrate[state
			] }
	end
	return message
end
...
function preserveTakePitchProperty:get()
	local message = initOutputMessage()
	message:initType("Toggle this property to set the switch status of preserving current take pitch when play rate changes of selected item."
		, "Toggleable")
	if multiSelectionSupport == true then
		message:addType(" If the group of items has been selected, the preserve state will be set to oposite value depending of moreness items with the same value."
			, 1)
	end
...
function takePitchProperty:get()
	...
	if istable(items) then
		...
		local state = self.getValue(items)
		message { objectId = string.format("%s %s", getItemID(items), getTakeID(items)), value = representation.pitch[state] }
	end
	return message
end
...
function takePitchShifterProperty:get()
	...
	if multiSelectionSupport == true then
		message:addType(string.format(' If the group of items has been selected, the value will enumerate only if selected items have the same value. Otherwise, the pitch shifter state will be set to "%s", then will enumerate this.'
			, self.states[-1]), 1)
	end
...
function takePitchShifterModeProperty:get()
	...
	if istable(items) then
		...
		message { objectId = string.format("%s %s", getItemID(items), getTakeID(items)), value = self.states[state] }
		if state == -1 then
			message:changeType(string.format("The Property is unavailable right now, because the shifter has been set to %s. Set the specified shifter before setting it up."
				, takePitchShifterProperty.states[-1]), 1)
...
function takePitchShifterModeProperty:set_adjust(direction)
	...
	if istable(items) then
		...
		for k = 1, #items do
			local state = self.getValue(items[k])
			if state == -1 then
				return string.format("The shifter of take %u %s is set to %s. Set any otherwise  shifter on this take before  setting up the shifter mode."
		...
		local state = self.getValue(items)
		if state == -1 then
			return string.format("The property is unavailable right now, because the shifter has been set to %s. Set the specified shifter before setting it up."
